...
// ㅇㅇ findUser: (authId: string)=> Promise<UserDto | null>; // user가 자신의 정보를 조회함
// // DTO에선 없지만 controller의 미들웨어 같은 걸로 email을 넣어주어야 함
// ㅇㅇ updateUser: (userInfo: Omit<UserT, 'role'>) => Promise<void>; // user가 자신의 정보를 수정하는 기능
// ddㅇㅇdeleteUser: (email: UserT['email'])=> Promise<void>; // 유저가 스스로 탈퇴할 수 있는 기능
// ㅇㅇ createUser: (userInfo: CreateUserDto) => Promise<void>; // register
// ㅇㅇfindUserByUsername: (userEmail: UserT['email']) => Promise<UserDto>;
