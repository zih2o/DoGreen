...
export class CommentRepository implements ICommentRepository {
  ...
  async isWrittenByCurrentUser(commentId: string, currentAuthId: UserT['auth']): Promise<void> {
    const targetCommentAuthId = await CommentModel.findById(commentId).select('authId');
    invariant(targetCommentAuthId !== null, new ForbiddenError('작성자가 아니므로 권한이 존재하지 않습니다.'));
  }
  ...
  async updateComment(commentId: string, toUpdate: updateCommentDto, currentAuthId: UserT['auth']) {
    await this.isWrittenByCurrentUser(commentId, currentAuthId);
