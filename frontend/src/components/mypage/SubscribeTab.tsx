...
import { useSubquery } from '../../hooks/useSubscription';
...
const SubscribeTab = () => {
  ...
  const tabCards = subInfo?.map((card, index) => (
    <li className={CardType.size + CardType.flipContent} key={subInfo[index]._id}>
      <div key={`back-${subInfo[index]._id}`} className={CardType.layout + CardType.back}>
        <button type="button" className={BtnType.newsLetterBtn}>
          {' '}
          <Link to={`/categories/${subInfo[index].categoryName}`}>뉴스레터 📰</Link>
        </button>
        <button
          type="button"
          className={BtnType.subCancelBtn}
          data-id={subInfo[index]._id}
          id={subInfo[index]._id}
          onClick={() => {
            setIsModal(!isModal);
          }}
        >
      ...
      <div key={`front-${subInfo[index]._id}`} className={CardType.layout + CardType.front}>
        <div className={CardType.imgWrapper}>
          <img className={CardType.img} src={subInfo[index].mascotImage} alt="default card" />
        </div>
        <div className={CardType.text}>
          <h2>
            <span className={TextType.mascotNameText}>{checkName(subInfo[index].mascotName)}</span> 전하는 <br></br>
            <span className={TextType.categoryNameText}>{subInfo[index].categoryName}</span>
          </h2>
        </div>
      </div>
    </li>
